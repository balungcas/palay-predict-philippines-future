...
const ForecastResults = ({ predictions, evaluation, method, latestProduction }: ForecastResultsProps) => {
            ...
            if (entry.dataKey === 'production') {
              return (
                <p key={index} style={{ color: '#567327' }}>
                  Predicted: {formatNumber(entry.value as number)} tonnes
                </p>
              );
        ...
        <CardContent className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="bg-rice-50 p-4 rounded-md border border-rice-100">
              <p className="text-sm text-rice-700">
                {predictions.length > 1 
                  ? `Forecast for Year ${predictions[predictions.length-1].year}`
                  : `Forecast for Year ${predictions[0].year}`}
              </p>
              <p className="text-xl font-semibold text-rice-800">
            ...
            <div className={`p-4 rounded-md border flex items-center ${
              ...
              <div>
                <p className="text-sm text-rice-700">Expected Growth</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
              {evaluation.mae !== undefined && (
                <div>
                  ...
                  <p className="font-medium">{formatNumber(evaluation.mae, 2)} tonnes</p>
                </div>
              )}
              {evaluation.rmse !== undefined && (
                <div>
                  <p className="text-rice-600">Root Mean Square Error (RMSE)</p>
                  <p className="font-medium">{formatNumber(evaluation.rmse, 2)} tonnes</p>
                </div>
              )}
