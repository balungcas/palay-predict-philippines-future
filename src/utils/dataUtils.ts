...
const parseFAOFormat = (lines: string[], headers: string[]): ParsedDataResult => { // Updated return type
  ...
  // Item column is now optional
  // if (itemIndex === -1 && itemCodeIndex === -1) missingColumns.push("Item or Item Code");
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (elementVal.includes('gross production') || elementVal === '152' || elementVal.includes('production')) { // Added general 'production'
